...
get_ref_roi_data <- function(ref_pts_roi, phrase_id) {
...
get_corr <- function(ref_pt, subject_pt, ref_it){
  ...
  #weightedCorr(ref_pt, subject_pt, method="pearson")
}
...
get_comparison_function <- function(filenames, pts, ref_pts, ref_its, method, same=TRUE) {
  function(index) {
    filename <- filenames[[index]]
    #print(filename)
    # Find the index of that ref phrase
    ref_index <- get_ref_index2(filename, same)
...
### 3.2 Scores for comparison with target reference
```{r}
## CORRELATION SCORES
### VOICE
...
mean(learners_voice_corr) 

# For tg2
get_corr_learners_voice2 <- get_comparison_function(learners_pt_filenames, learners_pts_roi2, 
                                                 ref_pts_roi2, ref_its_roi2, get_corr) 
learners_voice_corr2 <- seq(1, length(learners_pt_filenames)) %>% sapply(get_corr_learners_voice2)
mean(learners_voice_corr2)
...
mean(natives_voice_corr) 

# For tg2
get_corr_natives_voice2 <- get_comparison_function(natives_pt_filenames, natives_pts_roi2, 
                                                ref_pts_roi2, ref_its_roi2, get_corr) 
natives_voice_corr2 <- seq(1, length(natives_pt_filenames)) %>% sapply(get_corr_natives_voice2)
mean(natives_voice_corr2) 
...
mean(learners_gest_corr) 

# For tg2
get_corr_learners_gest2 <- get_comparison_function(learners_gest_filenames, learners_gests_roi2,
                                                  ref_pts_roi2, ref_its_roi2, get_corr)
learners_gest_corr2 <- seq(1, length(learners_gest_filenames)) %>% sapply(get_corr_learners_gest2)
mean(learners_gest_corr2) 
...
mean(natives_gest_corr) 

# For tg2
get_corr_natives_gest2 <- get_comparison_function(natives_gest_filenames, natives_gests_roi2,
                                                  ref_pts_roi2, ref_its_roi2, get_corr)
natives_gest_corr2 <- seq(1, length(natives_gest_filenames)) %>% sapply(get_corr_natives_gest2)
mean(natives_gest_corr2) 
```

### 3.3. Scores for comparison with non-target reference

```{r}
# Function that gets the weighted Pearson's Correlation scores for LEANERS Voice with Nontarget reference
get_corr_learners_voice_nontarget <- get_comparison_function(learners_pt_filenames,
                                                             learners_pts_roi_nontarget, 
                                                             ref_pts_roi, ref_its_roi, get_corr, FALSE) 
learners_voice_corr_nontarget <- seq(1, length(learners_pt_filenames)) %>%
                                 sapply(get_corr_learners_voice_nontarget)
mean(learners_voice_corr_nontarget) 

# Function that gets the weighted Pearson's Correlation scores for NATIVES Voice
get_corr_natives_voice_nontarget <- get_comparison_function(natives_pt_filenames, 
                                                            natives_pts_roi_nontarget, 
                                                            ref_pts_roi, ref_its_roi, get_corr, FALSE) 
natives_voice_corr_nontarget <- seq(1, length(natives_pt_filenames)) %>%
                                sapply(get_corr_natives_voice_nontarget)
mean(natives_voice_corr_nontarget) 

# Function that gets the weighted Pearson's Correlation scores for LEANERS Gestures
get_corr_learners_gest_nontarget <- get_comparison_function(learners_gest_filenames,
                                                            learners_gests_roi_nontarget,
                                                            ref_pts_roi, ref_its_roi, get_corr, FALSE)
learners_gest_corr_nontarget <- seq(1, length(learners_gest_filenames)) %>% 
                      sapply(get_corr_learners_gest_nontarget)
mean(learners_gest_corr_nontarget) 

# Function that gets the weighted Pearson's Correlation scores for NATIVES Gestures
get_corr_natives_gest_nontarget <- get_comparison_function(natives_gest_filenames, 
                                                 natives_gests_roi_nontarget,
                                                 ref_pts_roi, ref_its_roi, get_corr, FALSE)
natives_gest_corr_nontarget <- seq(1, length(natives_gest_filenames)) %>%
                               sapply(get_corr_natives_gest_nontarget)
mean(natives_gest_corr_nontarget) 
...
learners_voice_scores <- learners_voice_info %>% add_column(corr=learners_voice_corr,
                                                            corr_nt=learners_voice_corr_nontarget,
                                                            corr_ne=learners_voice_corr2)
natives_voice_scores <- natives_voice_info %>% add_column(corr=natives_voice_corr,
                                                          corr_nt=natives_voice_corr_nontarget,
                                                          corr_ne=natives_voice_corr2)
learners_gest_scores <- learners_gest_info %>% add_column(corr=learners_gest_corr,
                                                          corr_nt=learners_gest_corr_nontarget,
                                                          corr_ne=learners_gest_corr2)
natives_gest_scores <- natives_gest_info %>% add_column(corr=natives_gest_corr,
                                                        corr_nt=natives_gest_corr_nontarget,
                                                        corr_ne=natives_gest_corr2)
...
```{r}
# Non-target correlation scores
all_scores %>% 
  ggplot(aes(x=type, y=corr_ne, fill=factor(native))) +
  geom_boxplot() + 
  labs(title="Correlation ROI without final consonant", x="Condition", y="Correlation score")
```
...
#### 4.2.1 Vocal scores: free reading vs. imitation (Correlation)
```{r}
...
```{r}
# TextGrids without final consonant
corr_free_ne <- all_scores %>% filter(type=="free") %>%
  ggplot(aes(x=pid, y=corr_ne, fill=factor(native))) +
  geom_boxplot() + 
  labs(title="Vocal Correlation - Free, ROI without final consonant", 
            x="Phrase", y="Correlation score") + 
  scale_fill_discrete(name = "Nativeness")

corr_imitation_ne <- all_scores %>% filter(type=="imitation") %>%
  ggplot(aes(x=pid, y=corr_ne, fill=factor(native))) +
  geom_boxplot() + 
  labs(title="Vocal Correlation - Imitation, ROI without final consonant", 
            x="Phrase", y="Correlation score") +
  scale_fill_discrete(name = "Nativeness")

corr_chironomy_ne <- all_scores %>% filter(type=="guide") %>%
  ggplot(aes(x=pid, y=corr_ne, fill=factor(native))) +
  geom_boxplot() + 
  labs(title="Gestural Correlation, ROI without final consonant", 
            x="Phrase", y="Correlation score") +
  scale_fill_discrete(name = "Nativeness")

grid.arrange(corr_free_ne, corr_imitation_ne, ncol=1)

```


```{r}
grid.arrange(corr_chironomy, corr_imitation, ncol=1)
```
```{r}
grid.arrange(corr_chironomy_ne, corr_imitation_ne, ncol=1)
```


#### 4.2.2 Imitation scores: vocal vs. chironomy (Correlation)
```{r}
# Correlation scores by phrase
...
```{r}
# Correlation scores by phrase
corr_learners_ne <- all_scores %>% filter(type!="free" & native==FALSE) %>%
  ggplot(aes(x=pid, y=corr_ne, fill=factor(type))) +
  geom_boxplot() + 
  labs(title="Correlation Scores, ROI without final consonant - Nonnative", 
            x="Phrase", y="Correlation score") + 
  scale_fill_manual(name = "Type", labels=c("Gestural", "Vocal"), values=wes_palette("Moonrise3"))

corr_natives_ne <- all_scores %>% filter(type!="free" & native==TRUE) %>%
  ggplot(aes(x=pid, y=corr_ne, fill=factor(type))) +
  geom_boxplot() + 
  labs(title="Correlation Scores, ROI without final consonant - Native", 
            x="Phrase", y="Correlation score") + 
  scale_fill_manual(name = "Type", labels=c("Gestural", "Vocals"), values=wes_palette("Moonrise3"))

grid.arrange(corr_learners_ne, corr_natives_ne, ncol=1)
```

### 4.3 Scores by subject

```{r}
# Correlation scores by subject
all_scores %>% filter(type!="free") %>%
  ggplot(aes(x=subject, y=corr, fill=factor(type))) +
  geom_boxplot() + 
  labs(title="Correlation Scores by subject", 
            x="Phrase", y="Correlation score") + 
  scale_fill_manual(name = "Type" , values=wes_palette("Moonrise2"))

```
```{r}
all_scores %>% filter(type!="free") %>%
  ggplot(aes(x=subject, y=corr_ne, fill=factor(type))) +
  geom_boxplot() + 
  labs(title="Correlation Scores by subject, ROI without final consonant", 
            x="Phrase", y="Correlation score") + 
  scale_fill_manual(name = "Type" , values=wes_palette("Moonrise2"))
