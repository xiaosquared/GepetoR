...
gest_path <- "../data/21_05-fallrise/gest_learners/"

# Load filenames for gest & textgrids for LEARNERS
learners_gest_filenames <- dir(paste0(gest_path, "gest"))
learners_tg_filenames <- dir(paste0(gest_path, "tg"))
# Use filenames to load actual gest (as JSON) and textgrids
learners_gests <- lapply(paste0(gest_path, "gest/", learners_gest_filenames), load_gest)
learners_gests_tgs <- lapply(paste0(gest_path, "tg/", learners_tg_filenames), load_textgrid)
...
get_corr <- function(ref_pt, subject_pt){
  cor(ref_pt, subject_pt, method="pearson")
}

get_rmse <- function(a, b) {
  return(rmse(predicted=(a-mean(a)), actual=(b-mean(b))))
}
...
get_comparison_function <- function(filenames, pts, ref_pts, method) {
  function(index) {
    ...
    # Run the method    
    method(ref_pt$f, pts[[index]]$f)
  }
} 


## CORRELATION SCORES
# Function that gets the non-weighted Pearson's Correlation scores for LEANERS Voice
get_corr_learners_voice <- get_comparison_function(learners_pt_filenames, learners_pts_roi, 
                                                 ref_pts_roi, get_corr) 
# The non-weighted Pearson's Correlation scores for LEARNERS
...
# Function that gets the non-weighted Pearson's Correlation scores for NATIVES Voice
get_corr_natives_voice <- get_comparison_function(natives_pt_filenames, natives_pts_roi, 
                                                ref_pts_roi, get_corr) 
# The non-weighted Pearson's Correlation scores for NATIVES
...
# Function that gets the non-weighted Pearson's Correlation scores for LEANERS Gestures
...
all_scores<-rbind(learners_voice_scores, natives_voice_scores, learners_gest_scores)
...
corr_chironomy <- all_scores %>% filter(type=="guide") %>%
  ggplot(aes(x=pid, y=corr, fill=factor(native))) +
  geom_boxplot() + 
  labs(title="Vocal Correlation with Reference - Chironomy", 
            x="Phrase", y="Correlation score") +
  scale_fill_discrete(name = "Nativeness")
