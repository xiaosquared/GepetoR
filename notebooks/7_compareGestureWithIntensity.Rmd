...
get_corr <- function(gest_name, weights=FALSE) {
  ...
  if (weights) {  
    ...
    return(cor(gest_pitch, prosogram_pitch, method="pearson"))
  }
}
...
plot_gest_and_ref_timing <- function(gest_name) {
  ...
  my_data <- tibble(gest=gest_scrub, ref=gest_index, percent=gest_index)
  
  my_plot <- ggplot(data=my_data, aes(x=percent, y=gest, color="gesture")) +
    scale_color_discrete(name = "Data source") +
    geom_line() +
    geom_line(data=my_data, aes(x=percent, y=ref, color="ideal")) +
