...
voice_comparison_data_tibble <- bind_rows(voice_comparison_data)  
...
# Native speakers
voice_data %>% mutate(pid = fct_relevel(pid, 
            "2", "2bis", "7", 
            "7bis", "8", "8bis", 
            "10", "10bis", "11", "11bis", "21", "21bis")) %>%
  ggplot(aes(x=pid, y=corr_psg, fill=factor(type))) +
  geom_boxplot() +
...
# Getting a better alignment

Might need to do dynamic time warping (dtw)... 

I'm skeptical about using DTW for this analysis because it gets rid of the actual timing of the pronunciations.
...
template <- voice_comparison_data[[voice_id]]$rec_f
reference <- voice_comparison_data[[voice_id]]$ref_f_psg 
alignment<-dtw(template, reference, keep=TRUE, step.pattern=asymmetric, open.begin=FALSE)
plot(alignment, type="twoway")

dtw_voice <- sapply(alignment$index2, function(i) {template[i]})

cdata <- voice_comparison_data[[voice_id]]
cdata <- cdata %>% add_column(dtw_f=dtw_voice)

ggplot(data=cdata, aes(x=percent_t, y=ref_f_psg, color="Reference")) +
            scale_color_discrete(name = "Data source") +
            geom_line(size=2) + 
            geom_line(mapping=aes(x=percent_t, y=rec_f, color="Original recording")) +
            geom_line(mapping=aes(x=percent_t, y=dtw_f, color="DTW")) +
